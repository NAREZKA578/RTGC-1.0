...
impl FontAtlas {
    pub fn new(gl: &glow::Context, font: &Font, size: f32) -> Result<Self> {
        ...
        let gl_texture = unsafe {
            ...
            gl.tex_image_2d(glow::TEXTURE_2D, 0, glow::R8 as i32, ATLAS_W as i32, ATLAS_H as i32, 0, glow::RED, glow::UNSIGNED_BYTE, Some(&pixels));
        ...
        Ok(Self { gl_texture, glyphs, line_height: max_h.max(30.0) })
    }
}
...
impl UiRenderer {
    ...
    pub fn push_text(&self, batch: &mut DrawBatch, text: &str, x: f32, y: f32, _size: f32, color: Color) {
        ...
        for ch in text.chars() {
            if let Some(info) = atlas.glyphs.get(&ch) {
                if info.w == 0 || info.h == 0 { cursor_x += info.advance; continue; }
    ...
    pub fn measure_text_width(&self, text: &str) -> f32 {
        ...
        let mut width = 0.0;
        for ch in text.chars() {
            if let Some(info) = atlas.glyphs.get(&ch) { width += info.advance; }
            else if ch == ' ' { width += 14.0; }
        }
        width
    }
}
